...
//    size_t Read(TYPE& a_value)
//    {
//        memcpy((void*)&a_value, buffer, sizeof(a_value));
//        buffer += sizeof(a_value);
//        return 1;
//        //ASSERT(m_file);
//        //return fread((void*)&a_value, sizeof(a_value), 1, m_file);
//    }
...
//    size_t ReadArray(TYPE* a_array, int a_count)
//    {
//        memcpy((void*)a_array, buffer, sizeof(TYPE) * a_count);
//        buffer += sizeof(TYPE) * a_count;
//        return 1;
//        //ASSERT(m_file);
//        //return fread((void*)a_array, sizeof(TYPE) * a_count, 1, m_file);
//    }
...
bool RTREE_QUAL::Load(const char* a_fileName)
{
  ...
  endtime = clock();
  //time.locationtime = endtime - starttime;
  starttime = clock();
  bool result = Load(stream);
  endtime = clock();
  //time.loadtime = endtime - starttime;
...
RTREE_TEMPLATE
bool RTREE_QUAL::CalnByte()
{
    return CalnByte(m_root);
}
...
RTREE_TEMPLATE
long RTREE_QUAL::CalnByte(Node* a_node)
{
   long sumByte = sizeof(int) * 2 + sizeof(long);
   if (a_node->IsInternalNode())  // not a leaf node
    {
        for (int index = 0; index < a_node->m_count; ++index)
        {
            sumByte += sizeof(double) * 4;
            Branch* curBranch = &a_node->m_branch[index];
            sumByte += CalnByte(curBranch->m_child);
        }
        a_node->nByte = sumByte;
    }
    else // A leaf node
    {
        for (int index = 0; index < a_node->m_count; ++index)
        {
            sumByte += sizeof(double) * 4 + sizeof(long long);
        }
        a_node->nByte = sumByte;
    }
    return sumByte; 
